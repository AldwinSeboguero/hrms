...
const getData = async (page) => {
    loading.value = true;
    try {
        const response = await axios.get(`/summary-rating-opcr?page=${page}`);
...
const searchName = async (search) => {
    loading.value = true;
    try {
        const response = await axios.get(`/summary-rating-opcr?search=${search}`);
...
const fetchSummaryRating = debounce(async (value) => {
    ...
    try {
        const response = await axios.get(`/summary-rating-opcr?year=${value}`);
